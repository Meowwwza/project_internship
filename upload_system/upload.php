...
if (isset($_POST['post'])) {
    ...
    for ($i = 0; $i < $file_count; $i++) {
        ...
        if ($uploadOk == 0) {
            ...
            if (move_uploaded_file($_FILES["pimage"]["tmp_name"][$i], $target_file)) {
                ...
                echo 
                    "ไฟล์ " . basename($_FILES["pimage"]["name"][$i]) . " ได้รับการอัปโหลดเรียบร้อยแล้ว.<br>";
            } else {
    ...
    if (!empty($uploaded_files)) {
        ...
        $sql = "INSERT INTO post (idUser, idCategory, pimage, head, detail, price, location) VALUES ('$idUser', '$idCategory', '$pimage', '$head', '$detail', '$price', '$location')";

        if ($conn->query($sql) === TRUE) {
            echo "<script>
                    alert('ข้อมูลได้รับการบันทึกลงในฐานข้อมูลเรียบร้อยแล้ว');
